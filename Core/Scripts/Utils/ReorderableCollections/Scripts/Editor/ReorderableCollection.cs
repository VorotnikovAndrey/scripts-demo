...
namespace ZeroVector.Common.Reorderable.Editor {
    public class ReorderableCollection {
        ...
        private const string EmptyLabel = "Collection is empty.";
        private const string ArrayError = "{0} is not a reorderable collection!";
        ...
        public delegate void ActionDelegate(ReorderableCollection collection);
        public delegate bool ActionBoolDelegate(ReorderableCollection collection);
        public delegate void AddDropdownDelegate(Rect buttonRect, ReorderableCollection collection);
        public delegate Object DragDropReferenceDelegate(Object[] references, ReorderableCollection collection);
        public delegate void DragDropAppendDelegate(Object reference, ReorderableCollection collection);
        public delegate float GetElementHeightDelegate(SerializedProperty element);
        public delegate float GetElementsHeightDelegate(ReorderableCollection collection);
        ...
        public delegate void SurrogateCallback(SerializedProperty element, Object objectReference, ReorderableCollection collection);
        ...
        // ReSharper disable FieldCanBeMadeReadOnly.Global, FieldCanBeMadeReadOnly.Local, MemberCanBePrivate.Global
        public bool canAdd;
        ...
        // ReSharper restore FieldCanBeMadeReadOnly.Global, FieldCanBeMadeReadOnly.Local, MemberCanBePrivate.Global
        ...
        //
        // public ReorderableList(SerializedProperty list, bool canAdd = true, bool canRemove = true, bool draggable = true,
        //     ElementDisplayType elementDisplayType = ElementDisplayType.Auto, string elementNameProperty = null, Texture elementIcon = null)
        //     : this(list, canAdd, canRemove, draggable, elementDisplayType, elementNameProperty, null, elementIcon) { }

        public ReorderableCollection(SerializedProperty list, bool canAdd = true, bool canRemove = true,
            bool draggable = true,
            ElementDisplayType elementDisplayType = ElementDisplayType.Auto, string elementNameProperty = null,
            string elementNameOverride = null, Texture elementIcon = null) {
            if (list == null) {
                throw new MissingListException();
            }
            else if (!list.isArray) {
                //check if user passed in a ReorderableArray, if so, that becomes the list object
                var array = list.FindPropertyRelative("items");
        ...
        public void DoLayoutList() {
            ...
            DoUIList(EditorGUI.IndentedRect(position), label);
        }

        public void DoUIList(Rect rect, GUIContent label) {
        ...
        */
        ...
        private void DrawElement(SerializedProperty element, Rect rect, bool selected, bool focused) {
            ...
            if (evt.GetTypeForControl(controlId) == EventType.ContextClick && rect.Contains(evt.mousePosition)) {
        ...
        private GUIContent GetElementLabel(SerializedProperty element, bool allowElementLabel) {
            ...
            var name = GetElementNameCallback != null 
                ? GetElementNameCallback(element) : 
        ...
        private static string GetElementName(SerializedProperty element, string nameProperty, string nameOverride) {
            if (!string.IsNullOrEmpty(nameOverride)) {
                ...
                const string arrayEndDelimiter = "]";
                const char arrayStartDelimiter = '[';

                if (!path.EndsWith(arrayEndDelimiter)) return nameOverride;
                
                var startIndex = path.LastIndexOf(arrayStartDelimiter) + 1;
                return $"{nameOverride} {path.Substring(startIndex, path.Length - startIndex - 1)}";
        ...
        private void HandleSingleContextClick(Event evt, SerializedProperty element) {
            ...
            HandleSharedContextClick(evt, menu, "Duplicate Element", "Delete Element", "Move Element");
        }
        ...
        private void HandleMultipleContextClick(Event evt) {
            ...
            HandleSharedContextClick(evt, menu, "Duplicate Elements", "Delete Elements", "Move Elements");
        }
        ...
        private static class Style {
            ...
            static Style() {
                ...
                PaginationHeader = new GUIStyle("RL Element") {border = new RectOffset(2, 3, 2, 3)};
                ElementBackground = new GUIStyle("RL Element") {border = new RectOffset(2, 3, 2, 3)};
                VerticalLabel = new GUIStyle(EditorStyles.label) {
                    alignment = TextAnchor.UpperLeft, contentOffset = new Vector2(10, 3)
                };
                BoxBackground = new GUIStyle("RL Background") {border = new RectOffset(6, 3, 3, 6)};
        ...
        private class ListSelection : IEnumerable<int> {
            ...
            public ListSelection Clone() {
                return new ListSelection(ToArray()) {firstSelected = firstSelected};
            }
        ...
        private static class ListSort {
            ...
            internal static void SortOnProperty(SerializedProperty list, int length, bool descending,
                ...
                BubbleSort(list, length, (p1, p2) => {
                    ...
                    if (a == null || b == null || a.propertyType != b.propertyType) return 0;
            ...
            private static int Compare(SerializedProperty p1, SerializedProperty p2, bool descending,
                ...
                switch (type) {
                    ...
                    case SerializedPropertyType.String:
                        return string.Compare(p1.stringValue, p2.stringValue, StringComparison.Ordinal);
        ...
        public struct Surrogate {
            ...
            public readonly SurrogateCallback callback;
            internal bool enabled;
            ...
            public void Invoke(SerializedProperty element, Object objectReference, ReorderableCollection collection) {
                if (element != null && callback != null) {
                    callback.Invoke(element, objectReference, collection);
                }
            }
        }
        ...
        private static class Internals {
            ...
            private static object[] GetParams(ref object[] parameters, int count) {
                return parameters ?? (parameters = new object[count]);
            }
        }
    }
}