...
public class ScaleAnimation : MonoBehaviour
{
    ...
    {
        _tweener?.Kill();
        _tweener = _target.DOScale(_enterVector, _enterDuration).SetEase(_enterEase).OnKill(() => _tweener = null);
    }

    public void Exit()
    {
        _tweener?.Kill();
        _tweener = _target.DOScale(_exitVector, _exitDuration).SetEase(_exitEase).OnKill(() => _tweener = null);
    }
}
